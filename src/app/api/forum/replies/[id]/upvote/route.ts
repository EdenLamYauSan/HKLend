/**
 * POST /api/forum/replies/[id]/upvote — Toggle like on a reply (one per IP).
 *
...
export async function GET(
  ...
  const liked = await redis.sismember(`forum:like:reply:${id}`, ip)
  ...
  if (redis) {
    ...
    if (alreadyLiked) {
      ...
      const updated = await db.forumReply.update({
        where: { id },
        data: { upvotes: { decrement: reply.upvotes > 0 ? 1 : 0 } },
        select: { id: true, upvotes: true },
      })
